#!/bin/sh
#
# LOCATION: 	$bwDir/projects/zvinkaDiss/
...
#
# CREATED:	20110506     by stowler@gmail.com http://brainwhere.googlecode.com
# LAST UPDATED:	20110506 by stowler@gmail.com
#
# DESCRIPTION:
# Localizes the voxels endorsed in the user-provided cluster mask,
# and provids *MEAN* of the -i intensityVolume values that appear in each intersection of mask and atlas
# 
   ...
   while read line; do # start rowRegion outerloop
     ...
     # for each subrowMask region execute this inner loop to produce what will be subrows to the rowMask region's main row:
        ...
        while read line; do
			...
			subrowRegion_meanInt=`fslstats ${tempDir}/${subrowMask}/intensityBin/intersection_${subrowRegionName}_${hem}-AND-${rowRegionName}_${hem}_intensityProduct.nii.gz -m`
			subrowRegion_sdInt=`fslstats ${tempDir}/${subrowMask}/intensityBin/intersection_${subrowRegionName}_${hem}-AND-${rowRegionName}_${hem}_intensityProduct.nii.gz -s`
			...
			# departure from the normal script: the normal maxInt and minInt columns are both replaced by meanInt and sdInt
                        ...
                        echo "${subrowRegion_rowRegion_pct}%==${subrowRegionLabel} ${subrowRegion_meanInt} ${subrowRegion_sdInt} ${subrowRegion_minIntXYZ} ${subrowRegion_maxIntXYZ}" >> ${tempDir}/intersectionsOf${rowRegionName}_${hem}.txt
                    fi
...
# output report to screen for excel import, and to $outFile (notice end of EVERY line here):
   ...
   echo "" | tee -a ${outFile}
   echo "MASK.INTENSITY.VALUE REGION.LABEL MICROLITERS.TOTAL %.OUTSIDE.OF.STANDARD.BRAIN MICROLITERS.LEFT.HEM MICROLITERS.RIGHT.HEM LATERALITY.INDEX COMPOSITION.OF.REGION.IN.LH MEAN.INTENSITY.LH SD.INTENSITY.LH MIN.INTENSITY.XYZ.LH MAX.INTENSITY.XYZ.LH COMPOSITION.OF.REGION.IN.RH MEAN.INTENSITY.RH SD.INTENSITY.RH MIN.INTENSITY.XYZ.RH MAX.INTENSITY.XYZ.RH" | tee -a ${outFile}
