...
data.long<-(read.csv("/tmp/tempLongVolumes2.csv"))
head(data.long)
str(data.long)
summary(data.long)
...
data.long <- transform(data.long, ulTot=(ulLeft+ulRight)
head(data.long)
summary(data.long)
...
data.long$roi<-factor(data.long$roi, levels=c("CROSSONifg","CROSSONlatMotor","CROSSONremainingLatFrontal"))
...
data.wide <- dcast(data.long, participant + group + session ~ roi, value_var="LI")
